#!/usr/bin/env bash
# Convenience deploy wrapper: ships the repo to the head node and runs
# gateway/remote-deploy.sh. Run from the repo root on a Vulcan login node.
# (Cluster-specific defaults below; override HEAD for a different cluster.)
#
#   ./deploy-aleph/deploy.sh                                    # deploy with rkhoja/aleph:latest
#   GATEWAY_IMAGE=rkhoja/aleph:gateway-68f01ba ./deploy-aleph/deploy.sh   # pin a CI tag
#   HEAD=<control-plane-IP> ./deploy-aleph/deploy.sh
#
...
cd "$(dirname "$0")/.."   # repo root (this script lives in deploy-aleph/)
