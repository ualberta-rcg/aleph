...
# 1. Basic chat
def t01():
    r = req("POST", "/v1/chat/completions", {"model":"qwen25-coder-32b","messages":[{"role":"user","content":"What is 2+2?"}],"max_tokens":100})
...
# 2. Streaming chat
def t02():
    with req("POST", "/v1/chat/completions", {"model":"qwen25-coder-32b","messages":[{"role":"user","content":"Count 1 to 3"}],"max_tokens":100,"stream":True}, stream=True) as r:
...
def t03():
    r = req("POST", "/v1/chat/completions", {"model":"qwen25-coder-32b","messages":[{"role":"user","content":"Capital of France?"}],"max_tokens":50,"temperature":0})
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code==200 else "FAIL", r.status_code, "OAI temp=0", f"content={safe_content(msg)}")

# 4. Temperature=0.7 + top_k=20 (code model default)
def t04():
    r = req("POST", "/v1/chat/completions", {"model":"qwen25-coder-32b","messages":[{"role":"user","content":"Say hello"}],"max_tokens":100,"temperature":0.7,"top_k":20})
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code==200 else "FAIL", r.status_code, "OAI temp=0.7 top_k=20", f"content={safe_content(msg)}")

# 5. Top_p sampling
def t05():
    r = req("POST", "/v1/chat/completions", {"model":"qwen25-coder-32b","messages":[{"role":"user","content":"Say hello"}],"max_tokens":100,"temperature":0.7,"top_p":0.8})
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code==200 else "FAIL", r.status_code, "OAI top_p=0.8", f"content={safe_content(msg)}")

# 6. Stop sequences
def t06():
    r = req("POST", "/v1/chat/completions", {"model":"qwen25-coder-32b","messages":[{"role":"user","content":"Count: 1, 2, 3, 4, 5, 6, 7"}],"max_tokens":100,"stop":["5"]})
...
# 7. System prompt
def t07():
    r = req("POST", "/v1/chat/completions", {"model":"qwen25-coder-32b","messages":[{"role":"system","content":"You are a pirate. Always speak like a pirate."},{"role":"user","content":"Hello!"}],"max_tokens":100})
    d = r.json(); msg = d["choices"][0]["message"]
...
def t08():
    r = req("POST", "/v1/chat/completions", {"model":"qwen25-coder-32b","messages":[{"role":"user","content":"What is the weather in Edmonton?"}],"tools":[{"type":"function","function":{"name":"get_weather","description":"Get weather for a city","parameters":{"type":"object","properties":{"city":{"type":"string"}},"required":["city"]}}}],"max_tokens":200})
    d = r.json(); msg = d["choices"][0]["message"]
...
# 9. Code generation (specialist capability)
def t09():
    r = req("POST", "/v1/chat/completions", {"model":"qwen25-coder-32b","messages":[{"role":"user","content":"Write a Python function that reverses a linked list."}],"max_tokens":500,"temperature":0.2})
    d = r.json(); msg = d["choices"][0]["message"]; c = msg.get("content","")
    has_code = "def " in c or "class " in c
    record("PASS" if r.status_code==200 and has_code else "FAIL", r.status_code, "OAI code generation", f"has_code={has_code} content={safe_content(msg)}")

# 10. Large max_tokens
def t10():
    r = req("POST", "/v1/chat/completions", {"model":"qwen25-coder-32b","messages":[{"role":"user","content":"Say hi"}],"max_tokens":32000})
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code==200 else "FAIL", r.status_code, "OAI max_tokens=32k", f"content={safe_content(msg,40)}")

# 11. Resources block
def t11():
    r = req("POST", "/v1/chat/completions", {"model":"qwen25-coder-32b","messages":[{"role":"user","content":"hi"}],"max_tokens":10})
...
# 12. No reasoning content (non-reasoning model)
def t12():
    r = req("POST", "/v1/chat/completions", {"model":"qwen25-coder-32b","messages":[{"role":"user","content":"Explain why the sky is blue in one sentence."}],"max_tokens":200})
    d = r.json(); msg = d["choices"][0]["message"]
...
def t13():
    r = req("POST", "/v1/messages", {"model":"qwen25-coder-32b","max_tokens":100,"messages":[{"role":"user","content":"What is 3+3? Just the number."}]})
...
def t14():
    with req("POST", "/v1/messages", {"model":"qwen25-coder-32b","max_tokens":100,"stream":True,"messages":[{"role":"user","content":"Say hi"}]}, stream=True) as r:
...
def t15():
    r = req("POST", "/v1/messages", {"model":"qwen25-coder-32b","max_tokens":100,"system":"You are a helpful coding assistant.","messages":[{"role":"user","content":"Write a hello world in Python."}]})
    d = r.json(); content = d.get("content",[{}])[0].get("text","")
...
def t16():
    r = req("POST", "/v1/messages", {"model":"qwen25-coder-32b","max_tokens":50,"temperature":0,"messages":[{"role":"user","content":"Capital of France?"}]})
    d = r.json(); content = d.get("content",[{}])[0].get("text","")
    record("PASS" if r.status_code==200 else "FAIL", r.status_code, "ANT temp=0", f"text={content[:60]}")

# 17. Anthropic with top_p + top_k
def t17():
    r = req("POST", "/v1/messages", {"model":"qwen25-coder-32b","max_tokens":50,"top_p":0.8,"top_k":20,"messages":[{"role":"user","content":"Say hi"}]})
    d = r.json(); content = d.get("content",[{}])[0].get("text","")
    record("PASS" if r.status_code==200 else "FAIL", r.status_code, "ANT top_p+top_k", f"text={content[:60]}")

# 18. Anthropic with stop_sequences
def t18():
    r = req("POST", "/v1/messages", {"model":"qwen25-coder-32b","max_tokens":100,"stop_sequences":["5"],"messages":[{"role":"user","content":"Count: 1,2,3,4,5,6,7"}]})
...
# 19. Anthropic with tools
def t19():
    r = req("POST", "/v1/messages", {"model":"qwen25-coder-32b","max_tokens":200,"messages":[{"role":"user","content":"Weather in Edmonton?"}],"tools":[{"name":"get_weather","description":"Get weather","input_schema":{"type":"object","properties":{"city":{"type":"string"}},"required":["city"]}}]})
...
# 20. Anthropic max_tokens truncation
def t20():
    r = req("POST", "/v1/messages", {"model":"qwen25-coder-32b","max_tokens":10,"messages":[{"role":"user","content":"Tell me a long story"}]})
...
# 21. Embed model via Anthropic (should reject)
def t21():
...
# 23. Model catalog capabilities (no reasoning, tools=true, no vision)
def t23():
    r = req("GET", "/v1/models")
    m = next((x for x in r.json().get("data",[]) if x["id"]=="qwen25-coder-32b"), None)
    if not m:
...
################################################################
print("\n" + "="*60, flush=True)
print("Qwen2.5-Coder-32B Comprehensive Gateway Test", flush=True)
print("="*60 + "\n", flush=True)
