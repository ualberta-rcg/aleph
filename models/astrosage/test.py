...
check("Basic chat", *post("/v1/chat/completions", {"model": model, "messages": [{"role": "user", "content": "What is a black hole? Brief answer."}], "max_tokens": 80, "temperature": 0.1}))

check("System prompt", *post("/v1/chat/completions", {"model": model, "messages": [{"role": "system", "content": "You are an astrophysics expert."}, {"role": "user", "content": "What is a neutron star?"}], "max_tokens": 80, "temperature": 0.7}))

check("High temperature", *post("/v1/chat/completions", {"model": model, "messages": [{"role": "user", "content": "Describe a supernova"}], "max_tokens": 80, "temperature": 1.5}))
...
check("Short max_tokens", *post("/v1/chat/completions", {"model": model, "messages": [{"role": "user", "content": "Tell me about dark matter"}], "max_tokens": 10}))

# Streaming — gateway forces no_stream, so it returns JSON not SSE
print()
print("=== Streaming (no_stream=true, expect JSON) ===")
code, body, ct = post("/v1/chat/completions", {"model": model, "messages": [{"role": "user", "content": "Say hi"}], "max_tokens": 20, "stream": True})
if code == 200:
    if "event-stream" in ct or (body and body.startswith("data:")):
        P += 1
        print(f"{chr(9989)} Streaming SSE (unexpected but ok)")
    else:
        P += 1
        print(f"{chr(9989)} Streaming returned JSON (no_stream=true, correct)")
else:
    F += 1
...
check("Anthropic basic", *post("/v1/messages", {"model": model, "messages": [{"role": "user", "content": "What is the Hubble constant? Brief answer."}], "max_tokens": 60}))

check("Anthropic system", *post("/v1/messages", {"model": model, "messages": [{"role": "user", "content": "Hello"}], "system": "Be concise.", "max_tokens": 50}))

# Anthropic streaming — no_stream=true, expect JSON
code, body, ct = post("/v1/messages", {"model": model, "messages": [{"role": "user", "content": "Hello"}], "max_tokens": 30, "stream": True})
if code == 200:
    P += 1
    print(f"{chr(9989)} Anthropic streaming: HTTP {code} (no_stream=true, non-SSE)")
else:
    F += 1
