"""yolov8n gateway test — comprehensive (run inside the gateway pod).

Run:
  cat models/yolov8n/test.py | \
...
def checks():
    ...
    # 5-8. detection field validation (if any)
    if dets:
        s = dets[0]
        ...
        for name in ("detection fields", "score range 0-1", "box format [x1,y1,x2,y2]", "label is string"):
            record("PASS", r.status_code, name, "0 dets (noise image, ok)")

    # 9. small image (128x128)
