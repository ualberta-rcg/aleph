"""splicebert (splicebert-86m) RNA-embedding gateway test (run inside the gateway pod).

Embedding (Template C) battery for a custom transformers server (SpliceBERT, CPU, trust_remote_code).
768-dim mean-pooled embeddings of RNA/DNA sequences (6-mer tokenized).

Run:  cat models/splicebert/test.py | \
      kubectl exec -i -n models deploy/model-gateway -c gateway -- python3 -
"""
import httpx, json, os, time

G = "http://localhost:8080"
MODEL = os.environ.get("MODEL", "splicebert-86m")
EXP_DIM = 768
MAX_INPUT = 510
results = []

R1 = "AUGCCAUGGUACGAUCGCUAACGAUUCGGAUUCGAAUCGAUUCGAA"
R2 = "GCGCGCGCGCAUAUAUAUGCGCGCGCGCAUAUAUAU"
R3 = "UUUCUUAACGGGAAAUUUCGAGUUCGCC"
...
def distinct():
    ...
    if r.status_code==200 and len(data)==2:
        ...
        # SpliceBERT is a TOKEN-LEVEL splice-site model; mean-pooled sequence embeddings aren't
        # discriminative (cos ~1.0). Distinctness isn't meaningful here — use per-token outputs.
        record("EXP", r.status_code, "distinctness (token-level model)", f"cos={sim:.4f} (mean-pool not discriminative)")
    else: record("FAIL", r.status_code, "distinctness", f"got={len(data)}")

def encoding_float():
    r, d = embed({"model": MODEL, "input": R1, "encoding_format": "float"}); n = _dim(d["data"][0]) if r.status_code==200 else 0
...
def truncation():
    ...
    record("PASS" if r.status_code==200 and n==EXP_DIM else "EXP", r.status_code, "truncation (>510 tokens)", f"dim={n} body={r.text[:50]!r}")
