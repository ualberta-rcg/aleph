"""mast3r gateway test — comprehensive.

Run externally via the gateway VIP + Tyk auth (preferred):
  GW_URL=http://<GATEWAY_VIP> TYK_KEY=<key> python3 models/mast3r/test.py
...
def call_match(body, timeout=300):
    return httpx.post(EP_MATCH, json=body, timeout=timeout, headers=_HEADERS)


def call_recon(body, timeout=300):
    return httpx.post(EP_RECON, json=body, timeout=timeout, headers=_HEADERS)
...
def checks():
    ...
    # 14. guard — bad model
    rg2 = httpx.post(EP_MATCH, json={"model": "fake-nope-999", "images": [img1, img2]}, timeout=60, headers=_HEADERS)
