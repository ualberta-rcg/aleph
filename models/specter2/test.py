...
P1 = "Title: Attention Is All You Need. Abstract: We propose a new network architecture based solely on attention mechanisms."
P2 = "Title: CRISPR-Cas9 Genome Editing. Abstract: A versatile tool for precise genome modification."
...
def wake_dim():
    for attempt in range(72):
        r, d = embed({"model": MODEL, "input": P1})
...
def batch():
    r, d = embed({"model": MODEL, "input": [P1, P2, P3]}); data = d.get("data", []); dims = [_dim(x) for x in data]
...
def usage():
    r, d = embed({"model": MODEL, "input": P1}); u = d.get("usage", {})
...
def distinct():
    r, d = embed({"model": MODEL, "input": ["quantum physics paper", "cooking recipes"]}); data = d.get("data", [])
...
def encoding_float():
    r, d = embed({"model": MODEL, "input": P1, "encoding_format": "float"}); n = _dim(d["data"][0]) if r.status_code==200 else 0
    record("PASS" if r.status_code==200 and n==EXP_DIM else "FAIL", r.status_code, "encoding_format=float", f"dim={n}")

def truncation():
    long_text = "Title: X. Abstract: " + "machine learning research " * 200
...
def guard_badmodel():
    r = req("POST", "/v1/embeddings", {"model": "fake-xyz", "input": P1})
...
print("="*66, flush=True); print(f"{MODEL} paper-embedding gateway test", flush=True); print("="*66, flush=True)
