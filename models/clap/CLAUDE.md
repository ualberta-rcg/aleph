...
## Migration fixes / gotchas
- Pre-migration installed **GPU** torch (cu126) but requested no GPU → switched to CPU torch.
...
## Validation
See [test.py](test.py). text dim=512; 440Hz sine classified as "pure tone" (0.99999).
Run externally: `GW_URL=http://<GATEWAY_VIP> TYK_KEY=<key> python3 models/clap/test.py`.
