"""ancient-greek-bert embedding gateway test (run inside the gateway pod).

Embedding (Template C) battery for a custom transformers server (Ancient Greek BERT, GPU).
768-dim [CLS]-pooled embeddings of Ancient/Byzantine Greek text, via the standard OpenAI
/v1/embeddings endpoint (added 2026-06-19; /v1/science/embed kept as secondary).

Run:  cat models/ancient-greek-bert/test.py | \
...
T1 = "Ἐν ἀρχῇ ἦν ὁ λόγος, καὶ ὁ λόγος ἦν πρὸς τὸν θεόν."
T2 = "Ὁ ἄνθρωπος φύσει πολιτικὸν ζῷον."
T3 = "Γνῶθι σεαυτόν."
...
def distinct():
    r, d = embed({"model": MODEL, "input": [T1, "modern english sentence about cooking"]}); data = d.get("data", [])
...
def truncation():
    long_text = "λόγος ἀρχή ἄνθρωπος " * 200
...
print("="*66, flush=True); print(f"{MODEL} embedding gateway test", flush=True); print("="*66, flush=True)
