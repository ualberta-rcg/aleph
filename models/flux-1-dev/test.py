...
image model through the default gateway (no model-specific gateway code):
  ...
  - /v1/images/generations: sizes (square + non-square), n>1, steps low/high,
    guidance_scale, max_sequence_length, true_cfg_scale + negative_prompt (FLUX is
    guidance-distilled: negative_prompt only applies with true_cfg_scale>1),
    seed determinism (+ different seed differs).
...
MODEL = os.environ.get("MODEL", "flux-1-dev")
...
def wake():
    ...
    for attempt in range(120):  # ~10 min cap (big model cold start)
        try:
...
def steps_high():
    gen({"model": MODEL, "prompt": "an intricate clockwork mechanism", "size": "512x512",
         "num_inference_steps": 30}, "gen high steps=30")

def guidance():
    gen({"model": MODEL, "prompt": "a serene zen garden", "size": "512x512",
         "num_inference_steps": 10, "guidance_scale": 5.0}, "gen guidance_scale=5.0")
...
def max_seq():
    ...
    gen({"model": MODEL, "prompt": long_prompt, "size": "512x512",
         "num_inference_steps": 10, "max_sequence_length": 512},
        "gen max_sequence_length=512 (long prompt)")

def true_cfg_negative():
    # FLUX is guidance-distilled: negative_prompt only takes effect with true_cfg_scale>1.
    gen({"model": MODEL, "prompt": "a city street", "negative_prompt": "cars, people",
         "true_cfg_scale": 2.0, "size": "512x512", "num_inference_steps": 12},
        "gen true_cfg_scale=2 + negative_prompt")
...
for t in [wake, size_default, size_small, size_nonsquare, multi_n, steps_high,
          guidance, max_seq, true_cfg_negative, seed_determinism, seed_differs, img2img,
