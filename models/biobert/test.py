"""biobert biomedical-embedding gateway test (run inside the gateway pod).

Embedding (Template C) battery for a custom transformers server (DMIS BioBERT v1.1, GPU).
Input = biomedical text; output = 768-dim mean-pooled embeddings.
Covers WAKE/dim, batch, model-echo, usage, distinctness, encoding_format, truncation,
guardrails, catalog.

Run:  cat models/biobert/test.py | \
      kubectl exec -i -n models deploy/model-gateway -c gateway -- python3 -
"""
import httpx, json, os, time

G = "http://localhost:8080"
MODEL = os.environ.get("MODEL", "biobert")
EXP_DIM = 768
MAX_INPUT = 512
results = []

T1 = "BRCA1 mutations increase the risk of breast and ovarian cancer."
T2 = "The patient was prescribed metformin for type 2 diabetes."
T3 = "Aspirin inhibits cyclooxygenase enzymes."
...
def distinct():
    r, d = embed({"model": MODEL, "input": ["oncology research", "cooking recipes"]})
    data = d.get("data", [])
...
def truncation():
    long_text = "biomedical research cancer treatment " * 200   # >512 tokens
