...
Q1 = "treatment for type 2 diabetes"
Q2 = "side effects of metformin"
Q3 = "BRCA1 gene mutations"
...
def wake_dim():
    for attempt in range(72):
        r, d = embed({"model": MODEL, "input": Q1})
...
def batch():
    r, d = embed({"model": MODEL, "input": [Q1, Q2, Q3]}); data = d.get("data", []); dims = [_dim(x) for x in data]
...
def usage():
    r, d = embed({"model": MODEL, "input": Q1}); u = d.get("usage", {})
...
def distinct():
    r, d = embed({"model": MODEL, "input": [Q1, Q2]}); data = d.get("data", [])
...
def truncation():
    long_q = "diabetes treatment metformin insulin " * 20   # >64 tokens
    r, d = embed({"model": MODEL, "input": long_q}); n = _dim(d["data"][0]) if (r.status_code==200 and d.get("data")) else 0
    record("PASS" if r.status_code==200 and n==EXP_DIM else "EXP", r.status_code, "truncation (>64 tokens)", f"dim={n} body={r.text[:50]!r}")
...
def guard_badmodel():
    r = req("POST", "/v1/embeddings", {"model": "fake-xyz", "input": Q1})
...
print("="*66, flush=True); print(f"{MODEL} medical-query-embedding gateway test", flush=True); print("="*66, flush=True)
