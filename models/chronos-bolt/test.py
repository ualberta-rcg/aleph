"""chronos-bolt test — POST /v1/forecast {values, horizon}.

Chronos-Bolt zero-shot forecast over a numeric series. Sanity: median/mean length == horizon, finite.
Run: GW_URL=https://inference.vulcan.alliancecan.ca TYK_KEY=<key> GW_INSECURE=1 MODEL=chronos-bolt python3 models/chronos-bolt/test.py
"""
...
MODEL = os.environ.get("MODEL", "chronos-bolt")
ENDPOINT = "/v1/forecast"
HORIZON = 12
PAYLOAD = {"model": MODEL, "values": [10, 12, 15, 14, 17, 20, 22, 25, 28, 30, 33, 36, 38, 41, 44, 47],
           "horizon": HORIZON, "num_samples": 20}
results = []
...
def shape(d):
    ok = {"median", "mean"} <= set(d)
    record("PASS" if ok else "FAIL", 200, "SHAPE", f"keys={sorted(d.keys())}")


def sanity(d):
    med = d.get("median") or []
    ok = isinstance(med, list) and len(med) == HORIZON and all(math.isfinite(v) for v in med)
    record("PASS" if ok else "FAIL", 200, "SANITY", f"median_len={len(med)} (horizon={HORIZON}) median[:3]={med[:3] if med else []}")
