...
# 1. Basic chat
def t01():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "What is 2+2? Answer with just the number."}], "max_tokens": 100, "temperature": 0.8})
...
# 3. Thinking with budget (medium = 4096 tokens)
def t03():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "What is 15*17?"}], "max_tokens": 200, "thinking_token_budget": 4096}, timeout=180)
    d = r.json(); msg = d["choices"][0]["message"]
    rc = msg.get("reasoning_content", "")
    record("PASS" if r.status_code == 200 else "FAIL", r.status_code, "OAI thinking (budget=4096)", f"content={safe_content(msg)} reasoning={'yes' if rc else 'no'}")

# 4. Thinking OFF (budget=0)
def t04():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "Say hello"}], "max_tokens": 50, "thinking_token_budget": 0})
    d = r.json(); msg = d["choices"][0]["message"]
    rc = msg.get("reasoning_content", "")
    record("PASS" if r.status_code == 200 else "FAIL", r.status_code, "OAI thinking OFF (budget=0)", f"content={safe_content(msg)} reasoning={'yes' if rc else 'no'}")

# 5. Thinking high budget (12288)
def t05():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "Solve: if x^2 - 5x + 6 = 0, what is x?"}], "max_tokens": 500, "thinking_token_budget": 12288}, timeout=180)
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code == 200 else "FAIL", r.status_code, "OAI thinking (budget=12288)", f"content={safe_content(msg)}")

# 6. Temperature=0 (deterministic)
def t06():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "What is the capital of France?"}], "max_tokens": 50, "temperature": 0, "thinking_token_budget": 0})
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code == 200 else "FAIL", r.status_code, "OAI temp=0 (no thinking)", f"content={safe_content(msg)}")

# 7. Stop sequences
def t07():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "Count: 1, 2, 3, 4, 5, 6, 7"}], "max_tokens": 50, "stop": ["5"], "thinking_token_budget": 0})
...
# 8. System prompt
def t08():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "system", "content": "You are a helpful math tutor."}, {"role": "user", "content": "What is 3+4?"}], "max_tokens": 100, "thinking_token_budget": 0})
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code == 200 else "FAIL", r.status_code, "OAI system prompt", f"content={safe_content(msg)}")

# 9. Large max_tokens
def t09():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "Say hi"}], "max_tokens": 8192, "thinking_token_budget": 0})
    d = r.json(); msg = d["choices"][0]["message"]
...
def t10():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "hi"}], "max_tokens": 20, "thinking_token_budget": 0})
...
def t13():
    r = req("POST", "/v1/messages", {"model": MODEL, "max_tokens": 50, "system": "You are a helpful math tutor.", "messages": [{"role": "user", "content": "What is 2+2?"}]})
    d = r.json(); content = d.get("content", [{}])[0].get("text", "")
    record("PASS" if r.status_code == 200 else "FAIL", r.status_code, "ANT system prompt", f"text={content[:60]}")

# 14. Anthropic thinking disabled (budget=0)
def t14():
    r = req("POST", "/v1/messages", {"model": MODEL, "max_tokens": 50, "messages": [{"role": "user", "content": "Say hello"}], "thinking": {"type": "disabled"}})
    d = r.json(); content = d.get("content", [{}])[0].get("text", "")
...
# 19. Embed model via Anthropic (should reject)
def t19():
...
################################################################
print("\n" + "=" * 60, flush=True)
print("Phi-4-Reasoning Gateway Test", flush=True)
print("=" * 60 + "\n", flush=True)
