...
check("System prompt", *post("/v1/chat/completions", {"model": model, "messages": [{"role": "system", "content": "You are a pirate."}, {"role": "user", "content": "Hello"}], "max_tokens": 80, "temperature": 0.7}))
...
check("Short max_tokens", *post("/v1/chat/completions", {"model": model, "messages": [{"role": "user", "content": "Tell me about computers"}], "max_tokens": 10}))

# Streaming
print()
print("=== Streaming ===")
code, body, ct = post("/v1/chat/completions", {"model": model, "messages": [{"role": "user", "content": "Say hi"}], "max_tokens": 20, "stream": True})
# This model supports streaming, so expect SSE
if code == 200:
    # urllib reads the whole body, SSE comes as text
...
# Tools - should be rejected (supports_tools=false)
print()
...
print()
print("=== Custom params ===")
check("top_p param", *post("/v1/chat/completions", {"model": model, "messages": [{"role": "user", "content": "Hello"}], "max_tokens": 30, "top_p": 0.5}))
check("repetition_penalty", *post("/v1/chat/completions", {"model": model, "messages": [{"role": "user", "content": "Hello"}], "max_tokens": 30, "repetition_penalty": 1.2}))
