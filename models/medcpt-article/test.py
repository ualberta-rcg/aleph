"""medcpt-article medical-document-embedding gateway test (run inside the gateway pod).

Embedding (Template C) battery for a custom transformers server (NCBI MedCPT-Article-Encoder, GPU).
768-dim [CLS] embeddings of medical documents (max 512 tokens). Server compliant. Pair with medcpt-query.

Run:  cat models/medcpt-article/test.py | \
      kubectl exec -i -n models deploy/model-gateway -c gateway -- python3 -
"""
import httpx, json, os, time

G = "http://localhost:8080"
MODEL = os.environ.get("MODEL", "medcpt-article")
EXP_DIM = 768
MAX_INPUT = 512
results = []

D1 = "Title: Metformin and Clinical Outcomes. Abstract: We assessed cardiovascular outcomes in patients with type 2 diabetes treated with metformin."
D2 = "Title: CRISPR Gene Therapy. Abstract: In vivo CRISPR editing reduces transthyretin amyloidosis."
D3 = "Title: SARS-CoV-2 Vaccines. Abstract: mRNA vaccines show high efficacy against severe disease."
...
def wake_dim():
    for attempt in range(72):
        r, d = embed({"model": MODEL, "input": D1})
...
def batch():
    r, d = embed({"model": MODEL, "input": [D1, D2, D3]}); data = d.get("data", []); dims = [_dim(x) for x in data]
...
def usage():
    r, d = embed({"model": MODEL, "input": D1}); u = d.get("usage", {})
...
def distinct():
    r, d = embed({"model": MODEL, "input": [D1, "The recipe calls for two cups of flour and fresh basil mixed into the dough."]}); data = d.get("data", [])
...
def truncation():
    long_doc = "Title: X. Abstract: " + "metformin diabetes cardiovascular outcomes " * 200
    r, d = embed({"model": MODEL, "input": long_doc}); n = _dim(d["data"][0]) if (r.status_code==200 and d.get("data")) else 0
...
def guard_badmodel():
    r = req("POST", "/v1/embeddings", {"model": "fake-xyz", "input": D1})
...
print("="*66, flush=True); print(f"{MODEL} medical-document-embedding gateway test", flush=True); print("="*66, flush=True)
for t in [wake_dim, batch, model_echo, usage, distinct, truncation, guard_chat_rejected, guard_badmodel, catalog]:
    try: t()
