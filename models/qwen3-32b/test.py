...
# 1. Basic chat - thinking enabled by default
def t01():
...
def t04():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "Say hello in 5 words."}], "max_tokens": 30, "chat_template_kwargs": {"enable_thinking": False}})
    d = r.json(); msg = d["choices"][0]["message"]
    has_r = bool(msg.get("reasoning") or msg.get("reasoning_content"))
...
# 5. Temperature=0 (deterministic, thinking off to avoid repetition issues)
def t05():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "What is the capital of France?"}], "max_tokens": 20, "temperature": 0, "chat_template_kwargs": {"enable_thinking": False}})
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code == 200 else "FAIL", r.status_code, "OAI temp=0", f"content={safe_content(msg)}")

# 6. Temperature=1.0 + top_k=20 (Qwen recommended non-thinking)
def t06():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "Say hello"}], "max_tokens": 20, "temperature": 1.0, "top_k": 20, "chat_template_kwargs": {"enable_thinking": False}})
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code == 200 else "FAIL", r.status_code, "OAI temp=1 top_k=20", f"content={safe_content(msg)}")

# 7. Top_p sampling
def t07():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "Say hello"}], "max_tokens": 20, "top_p": 0.8, "chat_template_kwargs": {"enable_thinking": False}})
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code == 200 else "FAIL", r.status_code, "OAI top_p=0.8", f"content={safe_content(msg)}")

# 8. Stop sequences
def t08():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "Count: 1, 2, 3, 4, 5, 6, 7"}], "max_tokens": 50, "stop": ["5"], "chat_template_kwargs": {"enable_thinking": False}})
...
# 9. System prompt
def t09():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "system", "content": "You are a pirate. Always speak like a pirate."}, {"role": "user", "content": "Hello!"}], "max_tokens": 30, "chat_template_kwargs": {"enable_thinking": False}})
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code == 200 else "FAIL", r.status_code, "OAI system prompt", f"content={safe_content(msg)}")

# 10. Tool calling
def t10():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "What is the weather in Edmonton?"}], "tools": [{"type": "function", "function": {"name": "get_weather", "description": "Get weather for a city", "parameters": {"type": "object", "properties": {"city": {"type": "string"}}, "required": ["city"]}}}], "max_tokens": 100, "chat_template_kwargs": {"enable_thinking": False}})
    d = r.json(); msg = d["choices"][0]["message"]
...
# 11. Large max_tokens (should accept without error)
def t11():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "Say hi"}], "max_tokens": 32000, "chat_template_kwargs": {"enable_thinking": False}})
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code == 200 else "FAIL", r.status_code, "OAI max_tokens=32k", f"content={safe_content(msg, 40)}")

# 12. Usage/token counts present
def t12():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "hi"}], "max_tokens": 10, "chat_template_kwargs": {"enable_thinking": False}})
...
def t18():
    r = req("POST", "/v1/messages", {"model": MODEL, "max_tokens": 20, "top_p": 0.9, "top_k": 20, "messages": [{"role": "user", "content": "Say hi"}]})
    d = r.json(); content = d.get("content", [{}])[0].get("text", "")
...
# 25. Vision rejected (model does not support vision)
def t25():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": [{"type": "text", "text": "Describe this image."}, {"type": "image_url", "image_url": {"url": "https://upload.wikimedia.org/wikipedia/commons/thumb/4/47/PNG_transparency_demonstration_1.png/280px-PNG_transparency_demonstration_1.png"}}]}], "max_tokens": 50, "chat_template_kwargs": {"enable_thinking": False}})
    d = r.json()
    if r.status_code == 200:
        # vLLM may accept but ignore the image — check if response mentions inability
        msg = d["choices"][0]["message"]
        record("PASS", r.status_code, "Guard: vision rejected", f"content={safe_content(msg)} (accepted but no vision)")
    else:
        record("PASS", r.status_code, "Guard: vision rejected", f"rejected as expected error={json.dumps(d)[:100]}")

################################################################
print("\n" + "=" * 60, flush=True)
print("Qwen3-32B Comprehensive Gateway Test", flush=True)
print("=" * 60 + "\n", flush=True)
