"""qwen3-32b comprehensive gateway test (run inside the gateway pod).

Qwen3-32B: dense 32.8B, effort mode with a binary `enable_thinking` switch (real off,
unlike gpt-oss). vLLM `--reasoning-parser qwen3`. Text-only, tool calling (hermes).

Covers the standardized gateway surface for a managed-thinking reasoning model:
  - WAKE: first request retries through the gateway's 503 model_starting cold-start.
  - OpenAI + Anthropic endpoints: streaming, system, temperature/top_p/top_k, stop,
    tools, max_tokens, usage, resources.
  - Thinking: ON (medium/high) exposes the `reasoning` field / Anthropic `thinking` block;
    OFF (reasoning_effort none / thinking disabled) strips it + caps tokens.
  - Meta-tasks (OpenWebUI title/tags/followups): short, no reasoning.
  - Guardrails: embed-via-Anthropic, bad model, catalog, vision rejected (text-only).

Run:  cat models/qwen3-32b/test.py | \
...
def wake():
    ...
    for attempt in range(72):  # ~6 min cap (TP2 cold start ~3-4 min)
        r = req("POST", "/v1/chat/completions", body)
...
# ── Thinking (the managed-thinking contract) ──────────────────────────────────
def think_on_medium():
    r, d, m = oai({"model": MODEL, "messages": [{"role": "user", "content": HARD}],
                  "reasoning_effort": "medium", "max_tokens": 16000, "temperature": 0.6, "top_p": 0.95})
    rc = _rc(m)
...
def think_on_high():
    r, d, m = oai({"model": MODEL, "messages": [{"role": "user", "content": HARD}],
                  "reasoning_effort": "high", "max_tokens": 16000, "temperature": 0.6, "top_p": 0.95})
    rc = _rc(m)
    record("PASS" if r.status_code == 200 and rc else "FAIL", r.status_code,
...
def think_budget():
    r, d, m = oai({"model": MODEL, "messages": [{"role": "user", "content": HARD}],
                  "thinking_token_budget": 2000, "max_tokens": 20000, "temperature": 0.6})
    rc = _rc(m); ct = (d.get("usage") or {}).get("completion_tokens", 0)
...
def think_stream():
    ...
    with req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": HARD}],
             "reasoning_effort": "medium", "max_tokens": 8000, "temperature": 0.6, "top_p": 0.95,
...
# ── Vision guard (qwen3-32b is text-only) ─────────────────────────────────────
def vision_rejected():
...
def ant_topk():
    r = req("POST", "/v1/messages", {"model": MODEL, "max_tokens": 20, "top_p": 0.8, "top_k": 20,
            "thinking": {"type": "disabled"}, "messages": [{"role": "user", "content": "Say hi"}]})
    d = r.json(); t = next((b.get("text", "") for b in d.get("content", []) if b.get("type") == "text"), "")
...
def ant_maxtok():
    r = req("POST", "/v1/messages", {"model": MODEL, "max_tokens": 10,
            "messages": [{"role": "user", "content": "Tell me a long story"}]})
    d = r.json()
    record("PASS" if r.status_code == 200 else "FAIL", r.status_code, "ANT max_tokens=10",
           f"stop_reason={d.get('stop_reason')} output_tokens={d.get('usage',{}).get('output_tokens')}")
...
for t in [wake, stream, temp0, temp_topk, top_p, stop_seq, system, tools_oai, max_tokens,
          ...
          ant_maxtok, ant_think_on, ant_think_off, guard_embed, guard_badmodel, catalog]:
    try:
        t()
