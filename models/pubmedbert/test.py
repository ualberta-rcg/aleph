"""pubmedbert biomedical-embedding gateway test (run inside the gateway pod).

Embedding (Template C) battery for a custom transformers server (Microsoft PubMedBERT, CPU).
...
Run:  cat models/pubmedbert/test.py | \
      kubectl exec -i -n models deploy/model-gateway -c gateway -- python3 -
"""
import httpx, json, os, time

G = "http://localhost:8080"
MODEL = os.environ.get("MODEL", "pubmedbert")
EXP_DIM = 768
MAX_INPUT = 512
results = []

T1 = "Metformin is the first-line oral therapy for type 2 diabetes mellitus."
T2 = "The CRISPR-Cas9 system enables precise genome editing."
T3 = "Statins lower LDL cholesterol by inhibiting HMG-CoA reductase."
...
def distinct():
    r, d = embed({"model": MODEL, "input": ["oncology pharmacology", "cooking recipes"]})
    data = d.get("data", [])
...
def truncation():
    long_text = "biomedical literature pharmacology treatment " * 200   # >512 tokens
