"""sd3-medium comprehensive gateway test (run inside the gateway pod).

Exercises the full OpenAI image surface for the Stable Diffusion 3 Medium KServe
custom-predictor image model through the default gateway (no model-specific gateway code):
  ...
  - /v1/images/generations: sizes (square + non-square), n>1, steps low/high,
    guidance_scale, negative_prompt (SD3 uses real CFG so it's honored directly),
    max_sequence_length, seed determinism (+ different seed differs).
  ...
  - Guardrails: bad model 404.

Run:  cat models/sd3-medium/test.py | \\
...
MODEL = os.environ.get("MODEL", "sd3-medium")
...
def size_default():
    gen({"model": MODEL, "prompt": "a mountain lake at sunrise", "num_inference_steps": 14},
        "gen default size 1024x1024", want=(1024, 1024))

def size_small():
    gen({"model": MODEL, "prompt": "a green leaf", "size": "512x512", "num_inference_steps": 10},
        "gen size 512x512", want=(512, 512))

def size_nonsquare():
    gen({"model": MODEL, "prompt": "a wide desert horizon", "size": "1024x768",
         "num_inference_steps": 12}, "gen non-square 1024x768", want=(1024, 768))

def multi_n():
    gen({"model": MODEL, "prompt": "a colorful hot air balloon", "n": 2, "size": "512x512",
         "num_inference_steps": 10}, "gen n=2 multiple images")
...
def guidance():
    gen({"model": MODEL, "prompt": "a serene zen garden", "size": "512x512",
         "num_inference_steps": 14, "guidance_scale": 4.5}, "gen guidance_scale=4.5")

def negative():
    # SD3 uses real CFG -> negative_prompt is honored directly.
    gen({"model": MODEL, "prompt": "a vibrant flower garden",
         "negative_prompt": "people, text, watermark, blurry", "size": "512x512",
         "num_inference_steps": 14}, "gen negative_prompt (real CFG)")

def max_seq():
    long_prompt = ("a richly detailed steampunk city at dusk with airships, brass towers, "
                   "glowing lanterns, cobblestone streets, and intricate clockwork machinery " * 4)
    gen({"model": MODEL, "prompt": long_prompt, "size": "512x512",
         "num_inference_steps": 12, "max_sequence_length": 512},
        "gen max_sequence_length=512 (long prompt)")

def seed_determinism():
    body = {"model": MODEL, "prompt": "a lighthouse on a cliff", "size": "512x512",
            "num_inference_steps": 14, "seed": 12345}
...
def img2img():
    src = gen({"model": MODEL, "prompt": "a plain blue circle", "size": "512x512",
               "num_inference_steps": 12}, "img2img: make source")
    if not src:
...
for t in [wake, size_default, size_small, size_nonsquare, multi_n, steps_high,
          guidance, negative, max_seq, seed_determinism, seed_differs, img2img,
