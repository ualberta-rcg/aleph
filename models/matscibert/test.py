...
T1 = "Perovskite solar cells achieve high power conversion efficiency."
T2 = "The lithium-ion battery cathode uses NMC chemistry."
T3 = "Graphene exhibits exceptional thermal conductivity."
...
def distinct():
    r, d = embed({"model": MODEL, "input": ["perovskite photovoltaics", "cooking recipes"]}); data = d.get("data", [])
...
def truncation():
    long_text = "materials science battery cathode " * 200
...
print("="*66, flush=True); print(f"{MODEL} materials-embedding gateway test", flush=True); print("="*66, flush=True)
