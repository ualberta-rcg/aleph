"""clinicalbert (clinicalbert-110m) clinical-embedding gateway test (run inside the gateway pod).
...
T1 = "Patient admitted with acute appendicitis; scheduled for laparoscopic appendectomy."
T2 = "The patient denies chest pain, shortness of breath, or palpitations."
T3 = "HbA1c elevated at 9.2%; consider starting basal insulin."
...
def distinct():
    r, d = embed({"model": MODEL, "input": ["cardiology admission note", "cooking recipes"]})
    data = d.get("data", [])
...
def truncation():
    long_text = "patient discharge medication followup " * 200   # >512 tokens
...
# ── run ───────────────────────────────────────────────────────────────────────
print("=" * 66, flush=True); print(f"{MODEL} clinical-embedding gateway test", flush=True)
print("=" * 66, flush=True)
