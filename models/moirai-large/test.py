"""moirai-large test — POST /v1/science/forecast {context, prediction_length}.

Salesforce Moirai 1.1-R-Large forecasts a numeric series (mean + quantiles). Sanity: mean length
== prediction_length, finite.
Run: GW_URL=https://inference.vulcan.alliancecan.ca TYK_KEY=<key> GW_INSECURE=1 MODEL=moirai-large python3 models/moirai-large/test.py
"""
...
MODEL = os.environ.get("MODEL", "moirai-large")
ENDPOINT = "/v1/science/forecast"
HORIZON = 24
PAYLOAD = {"model": MODEL, "context": [10 + i * 0.7 + (i % 5) for i in range(96)],
           "prediction_length": HORIZON, "num_samples": 50}
results = []
...
def shape(d):
    ok = isinstance(d.get("mean"), list) and isinstance(d.get("quantiles"), dict)
