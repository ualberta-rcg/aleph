"""biomed-roberta biomedical-embedding gateway test (run inside the gateway pod).

Embedding (Template C) battery for a custom transformers server (BioMed-RoBERTa-base, GPU).
768-dim mean-pooled embeddings of biomedical text. Server is OpenAI-compliant (batch + usage).

Run:  cat models/biomed-roberta/test.py | \
      kubectl exec -i -n models deploy/model-gateway -c gateway -- python3 -
"""
import httpx, json, os, time

G = "http://localhost:8080"
MODEL = os.environ.get("MODEL", "biomed-roberta")
EXP_DIM = 768
MAX_INPUT = 512
results = []

T1 = "Lisinopril is an ACE inhibitor used to treat hypertension."
T2 = "Statin therapy reduces cardiovascular morbidity in high-risk patients."
T3 = "Sepsis is a life-threatening dysregulated host response to infection."
...
def truncation():
    long_text = "biomedical pharmacology clinical trial " * 200
...
print("="*66, flush=True); print(f"{MODEL} biomedical-embedding gateway test", flush=True); print("="*66, flush=True)
