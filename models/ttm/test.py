"""ttm test — POST /v1/science/forecast {context, prediction_length}. IBM TinyTimeMixer forecast."""
...
MODEL = os.environ.get("MODEL", "ttm"); EP = "/v1/science/forecast"
PL = {"model": MODEL, "context": [10+i*0.7+(i%5) for i in range(512)], "prediction_length": 96}
res = []
...
def shape(d):
    ok=isinstance(d.get("forecast"),list); rec("PASS" if ok else "FAIL",200,"SHAPE",f"keys={sorted(d.keys())}")
def sanity(d):
    fc=d.get("forecast") or []
    # flatten nested if needed
    if fc and isinstance(fc[0],list): fc=fc[0]
    ok=len(fc)>0 and all(math.isfinite(v) for v in fc)
    rec("PASS" if ok else "FAIL",200,"SANITY",f"len={len(fc)} first={fc[:3] if fc else []}")
