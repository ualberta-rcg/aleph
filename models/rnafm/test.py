"""rnafm RNA-embedding gateway test (run inside the gateway pod).

Embedding (Template C) battery for a custom server (multimolecule RNA-FM, GPU).
Mean-pooled RNA embeddings. OpenAI /v1/embeddings added 2026-06-19; /v1/science/embed kept.

Run:  cat models/rnafm/test.py | kubectl exec -i -n models deploy/model-gateway -c gateway -- python3 -
"""
import httpx, json, os, time

G = "http://localhost:8080"
MODEL = os.environ.get("MODEL", "rnafm")
EXP_DIM = 640   # RNA-FM 100M hidden size
MAX_INPUT = 1024
results = []
