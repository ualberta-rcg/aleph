"""timer test — POST /v1/forecast {time_series, prediction_length}. Timer zero-shot forecast."""
...
MODEL = os.environ.get("MODEL", "timer"); EP = "/v1/forecast"
...
def shape(d): rec("PASS" if isinstance(d.get("forecast"),list) else "FAIL",200,"SHAPE",f"keys={sorted(d.keys())}")
def sanity(d):
