...
# 4. Thinking toggle OFF
def t04():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "What is 2+2?"}], "max_tokens": 50, "thinking": False})
    d = r.json(); msg = d["choices"][0]["message"]
    rc = msg.get("reasoning_content", "")
...
# 8. Tool calling (glm45 parser)
def t08():
    r = req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": "What's the weather in Tokyo?"}], "max_tokens": 100, "tools": [{"type": "function", "function": {"name": "get_weather", "description": "Get current weather", "parameters": {"type": "object", "properties": {"location": {"type": "string"}}, "required": ["location"]}}}], "thinking": False})
    d = r.json(); msg = d["choices"][0]["message"]
...
# 13. Anthropic thinking disabled
def t13():
...
# 15. Anthropic with stop_sequences
def t15():
...
# 16. Anthropic tool calling
def t16():
    r = req("POST", "/v1/messages", {"model": MODEL, "max_tokens": 100, "messages": [{"role": "user", "content": "What's the weather in Tokyo?"}], "tools": [{"type": "custom", "name": "get_weather", "description": "Get current weather", "input_schema": {"type": "object", "properties": {"location": {"type": "string"}}, "required": ["location"]}}], "thinking": {"type": "disabled"}})
...
################################################################
print("\n" + "=" * 60, flush=True)
print("GLM-Z1-32B Gateway Test", flush=True)
print("=" * 60 + "\n", flush=True)
