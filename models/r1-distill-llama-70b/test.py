"""r1-distill-llama-70b comprehensive gateway test (run inside the gateway pod).

DeepSeek-R1-Distill-Llama-70B (TP4 whole node). **Always-on** reasoning (deepseek_r1 parser).
No tools, text-only. Always-on variant: ON exposes reasoning; OFF/meta strips + caps.

Run:  cat models/r1-distill-llama-70b/test.py | \
...
def wake():
    ...
    for attempt in range(84):  # TP4 cold start ~4-5 min
        r = req("POST", "/v1/chat/completions", body)
...
def max_tokens():
    r, d, m = oai({"model": MODEL, "messages": [{"role": "user", "content": "Say hi"}], "max_tokens": 8192})
...
def think_on():
    r, d, m = oai({"model": MODEL, "messages": [{"role": "user", "content": HARD}], "max_tokens": 16000})
    rc = _rc(m)
...
def think_stream():
    ...
    with req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": HARD}],
             "max_tokens": 8000, "stream": True, "stream_options": {"include_usage": True}}, stream=True) as r:
...
print("=" * 66, flush=True); print(f"{MODEL} comprehensive gateway test (always-on, TP4)", flush=True)
print("=" * 66, flush=True)
