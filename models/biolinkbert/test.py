"""biolinkbert biomedical-embedding gateway test (run inside the gateway pod).

Embedding (Template C) battery for a custom transformers server (BioLinkBERT-base, GPU).
768-dim mean-pooled embeddings of biomedical text. Server is OpenAI-compliant (batch + usage).

Run:  cat models/biolinkbert/test.py | \
      kubectl exec -i -n models deploy/model-gateway -c gateway -- python3 -
"""
import httpx, json, os, time

G = "http://localhost:8080"
MODEL = os.environ.get("MODEL", "biolinkbert")
EXP_DIM = 768
MAX_INPUT = 512
results = []

T1 = "BRCA1 mutations predispose women to breast and ovarian cancer."
T2 = "Metformin reduces hepatic glucose production in type 2 diabetes."
T3 = "ACE2 is the entry receptor for SARS-CoV-2."
...
def truncation():
    long_text = "biomedical literature cancer pharmacology " * 200
