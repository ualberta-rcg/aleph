"""scincl scientific-embedding gateway test (run inside the gateway pod).

Embedding (Template C) battery for a custom transformers server (ScINCL, GPU).
768-dim [CLS]-pooled embeddings of scientific text. Server is OpenAI-compliant.

Run:  cat models/scincl/test.py | \
      kubectl exec -i -n models deploy/model-gateway -c gateway -- python3 -
"""
import httpx, json, os, time

G = "http://localhost:8080"
MODEL = os.environ.get("MODEL", "scincl")
EXP_DIM = 768
MAX_INPUT = 512
results = []

T1 = "Title: Attention Is All You Need. Abstract: A transformer architecture based solely on attention."
T2 = "Title: CRISPR-Cas9. Abstract: A versatile tool for precise genome editing."
T3 = "Title: Graph Neural Networks. Abstract: Neural networks that operate on graph structures."
...
def distinct():
    r, d = embed({"model": MODEL, "input": [T1, "The recipe calls for two cups of flour and fresh basil mixed into the dough."]}); data = d.get("data", [])
...
def truncation():
    long_text = "Title: X. Abstract: " + "scientific research machine learning " * 200
    r, d = embed({"model": MODEL, "input": long_text}); n = _dim(d["data"][0]) if (r.status_code==200 and d.get("data")) else 0
...
print("="*66, flush=True); print(f"{MODEL} scientific-embedding gateway test", flush=True); print("="*66, flush=True)
