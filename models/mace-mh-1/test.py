"""MACE-MH-1 force-field test — POST /v1/science/predict {elements, positions, lattice, head}.

Predicts energy + per-atom forces (+ stress for periodic) for an atomistic structure. Sanity:
energy is a finite negative number (eV-scale), forces shaped [n_atoms][3] and finite.
Run: GW_URL=https://inference.vulcan.alliancecan.ca TYK_KEY=<key> GW_INSECURE=1 MODEL=mace-mh-1 python3 models/mace-mh-1/test.py
"""
...
MODEL = os.environ.get("MODEL", "mace-mh-1")
ENDPOINT = "/v1/science/predict"

# Cu FCC primitive-ish cell, 4 atoms (periodic → also exercises stress).
PAYLOAD = {
    "model": MODEL,
...
def sanity(d):
    ...
    ok = (isinstance(e, (int, float)) and math.isfinite(e) and e < 0
