"""megadetector gateway test (run inside the gateway pod)."""
import base64
import httpx
import os
import struct
...
MODEL = os.environ.get("MODEL", "megadetector")
results = []
...
def png_b64(seed=14, w=640, h=640):
    raw = bytearray()
    s = seed & 0x7FFFFFFF
    for _ in range(h):
        raw.append(0)
...
def wake():
    body = {"model": MODEL, "images": [png_b64(1)], "threshold": 0.2}
    for attempt in range(90):
        r = call(body, timeout=120)
        if r.status_code == 200:
            d = r.json()
            ok = isinstance(d.get("detections"), list)
            record("PASS" if ok else "FAIL", r.status_code, "WAKE + schema", f"attempts={attempt+1} batches={len(d.get('detections', []))}")
            return
...
def checks():
    r = call({"model": MODEL, "images": [png_b64(2)], "threshold": 0.2})
    d = r.json() if r.headers.get("content-type", "").startswith("application/json") else {}
    dets = d.get("detections", [])
    ok = r.status_code == 200 and isinstance(dets, list)
    if ok and dets and dets[0]:
        ok = all(k in dets[0][0] for k in ("category", "bbox", "conf"))
    record("PASS" if ok else "FAIL", r.status_code, "detect response shape", f"batches={len(dets)}")

    rm = call({"model": "fake-vision-nope", "images": [png_b64(3)]}, timeout=60)
