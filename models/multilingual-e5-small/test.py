...
T1 = "Machine learning improves search relevance."
T2 = "El aprendizaje automático mejora la relevancia de búsqueda."
T3 = "机器学习提升了搜索的相关性。"
...
def distinct():
    r, d = embed({"model": MODEL, "input": ["physics research", "cooking recipes"]})
    data = d.get("data", [])
...
def multilingual():
    # three languages, same meaning -> should cluster (cos high) but all be 384-dim
    r, d = embed({"model": MODEL, "input": [T1, T2, T3]})
    data = d.get("data", [])
    if r.status_code == 200 and len(data) == 3:
        c12 = _cos(data[0]["embedding"], data[1]["embedding"])
        record("PASS" if all(_dim(x) == EXP_DIM for x in data) else "FAIL", r.status_code,
               "multilingual (en/es/zh)", f"en-es cos={c12:.3f}")
    else:
        record("FAIL", r.status_code, "multilingual", f"got={len(data)}")
...
def truncation():
    long_text = "multilingual search embedding " * 200   # >512 tokens
...
# ── run ───────────────────────────────────────────────────────────────────────
print("=" * 66, flush=True); print(f"{MODEL} multilingual-embedding gateway test", flush=True)
print("=" * 66, flush=True)
for t in [wake_dim, batch, model_echo, usage, distinct, multilingual, encoding_float,
