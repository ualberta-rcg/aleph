"""biogpt test — POST /v1/completions {prompt, max_tokens}.

BioGPT continues a biomedical prompt (PubMed-trained). Sanity: choices non-empty, text is a
non-trivial continuation (len > 5).
Run: GW_URL=https://inference.vulcan.alliancecan.ca TYK_KEY=<key> GW_INSECURE=1 MODEL=biogpt python3 models/biogpt/test.py
"""
...
MODEL = os.environ.get("MODEL", "biogpt")
ENDPOINT = "/v1/completions"
PAYLOAD = {"model": MODEL, "prompt": "The treatment of diabetes includes", "max_tokens": 40, "temperature": 0.7}
results = []
...
def shape(d):
    ok = isinstance(d.get("choices"), list) and len(d["choices"]) > 0 and "text" in d["choices"][0]
    record("PASS" if ok else "FAIL", 200, "SHAPE", f"keys={sorted(d.keys())}")


def sanity(d):
    text = (d.get("choices") or [{}])[0].get("text", "")
    ok = isinstance(text, str) and len(text.strip()) > 5
    record("PASS" if ok else "FAIL", 200, "SANITY", f"text_len={len(text)} text[:60]={text[:60]!r}")
...
if __name__ == "__main__":
    print("=" * 66, flush=True); print(f"{MODEL} completions test ({ENDPOINT})", flush=True)
    print("=" * 66, flush=True)
    d = wake()
    if d:
