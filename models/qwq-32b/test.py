...
# 1. Basic chat with reasoning (QwQ always thinks)
def t01():
    r = req("POST", "/v1/chat/completions", {"model":"qwq-32b","messages":[{"role":"user","content":"What is 2+2?"}],"max_tokens":200,"temperature":0.6})
...
# 2. Streaming chat
def t02():
    with req("POST", "/v1/chat/completions", {"model":"qwq-32b","messages":[{"role":"user","content":"Count 1 to 3"}],"max_tokens":100,"temperature":0.6,"stream":True}, stream=True) as r:
...
def t03():
    r = req("POST", "/v1/chat/completions", {"model":"qwq-32b","messages":[{"role":"user","content":"What is the capital of France?"}],"max_tokens":100,"temperature":0})
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code==200 else "FAIL", r.status_code, "OAI temp=0", f"content={safe_content(msg)}")

# 4. Temperature=0.6 + top_k=40 (QwQ recommended)
def t04():
    r = req("POST", "/v1/chat/completions", {"model":"qwq-32b","messages":[{"role":"user","content":"Say hello"}],"max_tokens":100,"temperature":0.6,"top_k":40})
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code==200 else "FAIL", r.status_code, "OAI temp=0.6 top_k=40", f"content={safe_content(msg)}")

# 5. Top_p sampling
def t05():
    r = req("POST", "/v1/chat/completions", {"model":"qwq-32b","messages":[{"role":"user","content":"Say hello"}],"max_tokens":100,"temperature":0.6,"top_p":0.95})
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code==200 else "FAIL", r.status_code, "OAI top_p=0.95", f"content={safe_content(msg)}")

# 6. Stop sequences
def t06():
    r = req("POST", "/v1/chat/completions", {"model":"qwq-32b","messages":[{"role":"user","content":"After thinking, count: 1, 2, 3, 4, 5, 6, 7"}],"max_tokens":200,"stop":["5"],"temperature":0.6})
...
# 7. System prompt
def t07():
    r = req("POST", "/v1/chat/completions", {"model":"qwq-32b","messages":[{"role":"system","content":"You are a pirate. Always speak like a pirate."},{"role":"user","content":"Hello!"}],"max_tokens":100,"temperature":0.6})
    d = r.json(); msg = d["choices"][0]["message"]
...
def t08():
    r = req("POST", "/v1/chat/completions", {"model":"qwq-32b","messages":[{"role":"user","content":"What is the weather in Edmonton?"}],"tools":[{"type":"function","function":{"name":"get_weather","description":"Get weather for a city","parameters":{"type":"object","properties":{"city":{"type":"string"}},"required":["city"]}}}],"max_tokens":200,"temperature":0.6})
    d = r.json(); msg = d["choices"][0]["message"]
...
# 9. Large max_tokens
def t09():
    r = req("POST", "/v1/chat/completions", {"model":"qwq-32b","messages":[{"role":"user","content":"Say hi"}],"max_tokens":32000,"temperature":0.6})
    d = r.json(); msg = d["choices"][0]["message"]
    record("PASS" if r.status_code==200 else "FAIL", r.status_code, "OAI max_tokens=32k", f"content={safe_content(msg,40)}")

# 10. Resources block
def t10():
    r = req("POST", "/v1/chat/completions", {"model":"qwq-32b","messages":[{"role":"user","content":"hi"}],"max_tokens":10,"temperature":0.6})
...
# 11. Reasoning content present (QwQ always thinks)
def t11():
    r = req("POST", "/v1/chat/completions", {"model":"qwq-32b","messages":[{"role":"user","content":"Explain why the sky is blue in one sentence."}],"max_tokens":300,"temperature":0.6})
    d = r.json(); msg = d["choices"][0]["message"]
    has_r = bool(msg.get("reasoning") or msg.get("reasoning_content"))
...
def t12():
    r = req("POST", "/v1/messages", {"model":"qwq-32b","max_tokens":100,"temperature":0.6,"messages":[{"role":"user","content":"What is 3+3? Just the number."}]})
...
def t13():
    with req("POST", "/v1/messages", {"model":"qwq-32b","max_tokens":100,"stream":True,"temperature":0.6,"messages":[{"role":"user","content":"Say hi"}]}, stream=True) as r:
...
def t14():
    r = req("POST", "/v1/messages", {"model":"qwq-32b","max_tokens":100,"temperature":0.6,"system":"You are a pirate.","messages":[{"role":"user","content":"Hello!"}]})
    d = r.json(); content = d.get("content",[{}])[0].get("text","")
...
def t15():
    r = req("POST", "/v1/messages", {"model":"qwq-32b","max_tokens":50,"temperature":0,"messages":[{"role":"user","content":"Capital of France?"}]})
    d = r.json(); content = d.get("content",[{}])[0].get("text","")
    record("PASS" if r.status_code==200 else "FAIL", r.status_code, "ANT temp=0", f"text={content[:60]}")

# 16. Anthropic with top_p + top_k
def t16():
    r = req("POST", "/v1/messages", {"model":"qwq-32b","max_tokens":50,"top_p":0.95,"top_k":40,"messages":[{"role":"user","content":"Say hi"}]})
    d = r.json(); content = d.get("content",[{}])[0].get("text","")
...
def t17():
    r = req("POST", "/v1/messages", {"model":"qwq-32b","max_tokens":200,"stop_sequences":["5"],"messages":[{"role":"user","content":"Count: 1,2,3,4,5,6,7"}]})
...
def t18():
    r = req("POST", "/v1/messages", {"model":"qwq-32b","max_tokens":200,"messages":[{"role":"user","content":"Weather in Edmonton?"}],"tools":[{"name":"get_weather","description":"Get weather","input_schema":{"type":"object","properties":{"city":{"type":"string"}},"required":["city"]}}]})
...
def t19():
    r = req("POST", "/v1/messages", {"model":"qwq-32b","max_tokens":10,"messages":[{"role":"user","content":"Tell me a long story"}]})
...
# 22. Model catalog capabilities (reasoning=true, tools=true, no vision)
def t22():
    r = req("GET", "/v1/models")
    m = next((x for x in r.json().get("data",[]) if x["id"]=="qwq-32b"), None)
    if not m:
        record("FAIL", 0, "Catalog entry", "not found"); return
    c = m.get("capabilities",{})
    ok = c.get("tools") and not c.get("vision") and c.get("reasoning")
...
def t23():
    r = req("POST", "/v1/chat/completions", {"model":"qwq-32b","messages":[{"role":"user","content":[{"type":"text","text":"Describe this image briefly."},{"type":"image_url","image_url":{"url":"https://upload.wikimedia.org/wikipedia/commons/thumb/4/47/PNG_transparency_demonstration_1.png/280px-PNG_transparency_demonstration_1.png"}}]}],"max_tokens":50})
    d = r.json()
...
################################################################
print("\n" + "="*60, flush=True)
print("QwQ-32B Comprehensive Gateway Test", flush=True)
print("="*60 + "\n", flush=True)
