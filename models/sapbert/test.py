"""sapbert biomedical-entity-embedding gateway test (run inside the gateway pod).

Embedding (Template C) battery for a custom transformers server (SapBERT, GPU).
768-dim [CLS] biomedical entity-mention embeddings (max 25 tokens). OpenAI /v1/embeddings added
2026-06-19; /v1/science/embed kept.

Run:  cat models/sapbert/test.py | kubectl exec -i -n models deploy/model-gateway -c gateway -- python3 -
"""
import httpx, json, os, time

G = "http://localhost:8080"
MODEL = os.environ.get("MODEL", "sapbert")
EXP_DIM = 768
MAX_INPUT = 25
results = []

E1 = "myocardial infarction"
E2 = "type 2 diabetes"
E3 = "breast cancer"
...
def wake_dim():
    for attempt in range(72):
        r, d = embed({"model": MODEL, "input": E1})
...
def batch():
    r, d = embed({"model": MODEL, "input": [E1, E2, E3]}); data = d.get("data", []); dims = [_dim(x) for x in data]
...
def usage():
    r, d = embed({"model": MODEL, "input": E1}); u = d.get("usage", {})
...
def distinct():
    r, d = embed({"model": MODEL, "input": [E1, E2]}); data = d.get("data", [])
...
def guard_badmodel():
    r = req("POST", "/v1/embeddings", {"model": "fake-xyz", "input": E1})
...
print("="*66, flush=True); print(f"{MODEL} biomedical-entity-embedding gateway test", flush=True); print("="*66, flush=True)
