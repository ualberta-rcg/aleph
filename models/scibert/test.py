...
T1 = "Transformers enable transfer learning for natural language processing."
T2 = "The mitochondrion is the powerhouse of the cell."
T3 = "We trained a convolutional network on ImageNet."
...
def truncation():
    long_text = "protein folding machine learning " * 200   # >512 tokens
    r, d = embed({"model": MODEL, "input": long_text})
    n = _dim(d["data"][0]) if (r.status_code == 200 and d.get("data")) else 0
    # tokenizer truncates to max_length=512 -> 200 + a 768-dim vector (no 500).
...
# ── run ───────────────────────────────────────────────────────────────────────
print("=" * 66, flush=True); print(f"{MODEL} scientific-embedding gateway test", flush=True)
print("=" * 66, flush=True)
