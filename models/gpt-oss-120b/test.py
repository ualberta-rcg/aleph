...
Covers the full standardized gateway surface for a managed-thinking reasoning model:
  - WAKE: first request retries through the gateway's 503 model_starting cold-start
    (run this with the model scaled to 0 + stop removed to exercise the wake path).
  - OpenAI + Anthropic endpoints: streaming, system, temperature/top_p/top_k, stop,
    tools, max_tokens, usage, resources.
  - Thinking: ON (effort) exposes the `reasoning` field / Anthropic `thinking` block;
    OFF (reasoning_effort none / thinking disabled) strips it + caps tokens; a caller
    thinking_token_budget fakes a cap on this effort model.
  - Meta-tasks (OpenWebUI title/tags/followups signals): short, no reasoning.
...
def think_stream():
    ...
    with req("POST", "/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": HARD}],
             "reasoning_effort": "medium", "max_tokens": 8000, "temperature": 0, "stream": True,
...
# ── Vision guard (gpt-oss is text-only) ───────────────────────────────────────
def vision_rejected():
...
def ant_topk():
    r = req("POST", "/v1/messages", {"model": MODEL, "max_tokens": 20, "top_p": 0.9, "top_k": 20,
            "thinking": {"type": "disabled"}, "messages": [{"role": "user", "content": "Say hi"}]})
    d = r.json(); t = next((b.get("text", "") for b in d.get("content", []) if b.get("type") == "text"), "")
