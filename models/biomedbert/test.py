"""biomedbert (biomedbert-110m) biomedical-embedding gateway test (run inside the gateway pod).

Embedding (Template C) battery for a custom transformers server (Microsoft BiomedBERT, CPU).
768-dim mean-pooled embeddings of biomedical text.
Covers WAKE/dim, batch, model-echo, usage, distinctness, encoding_format, truncation,
guardrails, catalog.

Run externally via the gateway VIP + Tyk auth (preferred):
  GW_URL=http://<GATEWAY_VIP> TYK_KEY=<key> python3 models/biomedbert/test.py

Run inside the gateway pod (legacy, no auth needed):
  cat models/biomedbert/test.py | \
...
MODEL = os.environ.get("MODEL", "biomedbert-110m")
EXP_DIM = 768
MAX_INPUT = 512
results = []

T1 = "Trastuzumab is a monoclonal antibody used to treat HER2-positive breast cancer."
T2 = "Hypertension is a major risk factor for cardiovascular disease."
T3 = "Insulin resistance underlies the pathogenesis of type 2 diabetes."
...
def distinct():
    r, d = embed({"model": MODEL, "input": ["oncology clinical trial", "cooking recipes"]})
    data = d.get("data", [])
...
def truncation():
    long_text = "clinical trial patient outcome treatment " * 200   # >512 tokens
...
# ── run ───────────────────────────────────────────────────────────────────────
print("=" * 66, flush=True); print(f"{MODEL} biomedical-embedding gateway test", flush=True)
print("=" * 66, flush=True)
