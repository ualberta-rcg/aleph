"""molformer molecular-embedding gateway test (run inside the gateway pod).

Embedding (Template C) battery for a custom transformers server (IBM MoLFormer-XL, GPU).
768-dim molecular embeddings from SMILES, via the standard OpenAI /v1/embeddings endpoint
(added 2026-06-19; /v1/science/embed kept as secondary).

Run:  cat models/molformer/test.py | kubectl exec -i -n models deploy/model-gateway -c gateway -- python3 -
"""
import httpx, json, os, time

G = "http://localhost:8080"
MODEL = os.environ.get("MODEL", "molformer")
EXP_DIM = 768
MAX_INPUT = 202
results = []

S1 = "CC(=O)Oc1ccccc1C(=O)O"          # aspirin
S2 = "CC(C)CC1NC(=O)C(N)CCC(=O)O"     # peptide-ish
S3 = "c1ccccc1"                        # benzene
...
def distinct():
    r, d = embed({"model": MODEL, "input": [S1, S3]}); data = d.get("data", [])
...
def truncation():
    long_smiles = "CCO" * 100   # >202 tokens
    r, d = embed({"model": MODEL, "input": long_smiles}); n = _dim(d["data"][0]) if (r.status_code==200 and d.get("data")) else 0
    record("PASS" if r.status_code==200 and n==EXP_DIM else "EXP", r.status_code, "truncation (>202 tokens)", f"dim={n} body={r.text[:50]!r}")
...
print("="*66, flush=True); print(f"{MODEL} molecular-embedding gateway test", flush=True); print("="*66, flush=True)
